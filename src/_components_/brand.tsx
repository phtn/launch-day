...
export const Brand = ({ className, title }: BrandProps) => {
  ...
  return (
    <Link href={'/'} className={cn('flex items-center gap-2 md:gap-6', className)}>
      ...
      <h1 className='text-base md:text-lg lg:text-xl font-exo uppercase'>
        ...
        <span className='not-italic font-black tracking-tight drop-shadow-lg text-gray-300'>{b}</span>
      </h1>
    </Link>
  )
}

export const MadStacks = ({ className, title }: BrandProps) => {
  const [a, b] = title?.split(' ') || ['', '']
  return (
