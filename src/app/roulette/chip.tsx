...
import React, { MouseEvent, useCallback, useId, useMemo } from 'react'
...
export function Chip({
  ...
  const uniqueId = useMemo(() => `chip-${id}-${chipId}`, [id, chipId])
  ...
  const suits = useMemo(
    () => ({
      spade: 'M10,2 C10,2 0,9 0,17 C0,23 5,25 10,30 C15,25 20,23 20,17 C20,9 10,2 10,2 Z',
      heart: 'M10,6 C8,0 0,0 0,8 C0,12 5,16 10,20 C15,16 20,12 20,8 C20,0 12,0 10,6 Z',
  ...
  return (
    <div className={`relative ${className}`} style={{ width: size, height: size }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className='drop-shadow-xl'>
        ...
        {/* Edge with pattern */}
        <circle cx={radius} cy={radius} r={radius} fill='none' stroke={edgeColor} strokeWidth={edgeWidth} />

        {/* Edge inlays with card suits */}
        {inlays.map((inlay, i) => (
          <g key={`${uniqueId}-inlay-${i}`} transform={`translate(${inlay.x}, ${inlay.y}) rotate(${inlay.rotation})`}>
            <circle cx='5' cy='0' r={edgeWidth * 0.4} fill='white' stroke={edgeColor} strokeWidth='2' />
            <use
...
export const ChipBet = ({ value }: { value: number }) => {
  const cover = useMemo(() => getCoverByValue(value) ?? '/chips/c1.png', [value])

  return (
    <div className={cn('relative bg-transparent flex items-center justify-center rounded-full size-14 p-0')}>
      <TChip cover={cover} />
      <span className='absolute font-bold -tracking-widest drop-shadow-sm'>{value}</span>
    </div>
  )
}
