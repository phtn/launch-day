...
import { RouletteBets, RouletteDisplayResult } from '@/lib/roulette/game'
    ...
    <div className='hidden lg:flex flex-col w-full text-panel-dark font-medium'>
      ...
      <div className='grid grid-cols-6'>
        ...
        <div className='flex items-center justify-center border border-l border-r-0'>
          <Image src='/svg/black-slot.svg' alt='black' width={80} height={32} />
        ...
        <div className='flex items-center justify-center border border-r-0 border-l'>
          ...
          <Image src='/svg/red-slot.svg' alt='red' width={80} height={32} />
        </div>
        <div className='flex items-center justify-center h-16 border border-r-0 border-neutral-300 text-neutral-200 text-3xl'>
          ODD
        </div>
...
interface HistoryListProps {
  onChangeHistoryFn: MouseEventHandler<HTMLButtonElement>
  data: DataListProps
}
  ...
  return (
    <div className='md:w-80 bg-panel relative rounded-xl p-3'>
      <div className='flex justify-between absolute right-0 items-center mb-4'>
        ...
        <button onClick={props.onChangeHistoryFn} className='btn btn-ghost btn-sm'>
...
const DataList = ({ history, resultsHistory, getNumberColor }: DataListProps) => {
  return (
    <div className='flex flex-col gap-2'>
      <div className='tabs'>
        ...
        <div className='tab-content h-150 p-4 overflow-y-auto'>
          {history.length === 0 ? (
            ...
            <div className='grid grid-cols-5 gap-2'>
              {history.map((num, index) => (
                <div
                  key={`history-${index}`}
                  className={`size-10 rounded-full font-abril flex items-center justify-center text-sm font-bold text-white border border-gray-800 ${getNumberColor(
                    num
                  )}`}>
                  {num}
                </div>
        ...
        <div className='tab-content max-h-150 overflow-y-auto'>
          {resultsHistory.length === 0 ? (
            ...
            <div className='space-y-2'>
              {resultsHistory.map((result) => (
                <div
                  key={`${result.finalNumber}-${result.timestamp}`}
                  className={`p-2 bg-linear-to-l to-transparent rounded-md ${
                    result.kind === 'win' ? 'from-avocado-light/40 via-avocado-light/20' : 'from-error/40 via-brood/20'
                  } flex items-center justify-between`}>
                  <div className='flex items-center gap-2'>
                    <div
                      className={`size-8 rounded-full flex items-center justify-center text-sm font-bold text-white ${getNumberColor(
                        result.finalNumber
                      )}`}>
                      {result.finalNumber}
                    </div>
                    <div className='text-white font-bold'>{result.kind === 'win' ? 'Win' : 'Loss'}</div>
                  </div>
                  <div className='flex flex-col items-end'>
                    <div className={`font-bold ${result.kind === 'win' ? 'text-avocado' : 'text-white'}`}>
                      {result.kind === 'win' ? `+${result.amount}` : `-${result.amount}`}
                    </div>
                    <div className='text-xs text-gray-300'>{formatTime(result.timestamp)}</div>
                  </div>
                </div>
              ))}
            </div>
...
export const Header = ({ isMobile, muted, toggleMute, replenishFn, credits }: HeaderProps) => (
  <div className='flex justify-between items-center px-4 h-16 mt-14 bg-zinc-600 md:h-20 py-1.5 md:py-3 border-b border-slate-800/20'>
    <div className='flex items-center gap-2'>
      {/* {isAutoPlaying && (
    ...
    <div className='md:w-48 w-48'>
      <div className='space-y-1 py-1 bg-panel/20 space-x-2 px-4 rounded-lg md:text-xl font-semibold justify-between items-end'>
        ...
        <div className='flex items-center justify-between font-exo'>
          ...
          <span className='text-gray-300 text-right'>{credits.toLocaleString()}</span>
        </div>
      </div>
    </div>
...
export const ZeroNumberCell = ({ onPlaceBet, onRemoveBet, getNumberColor, selectedBets }: ZeroNumberCellProps) => (
  <div className='relative flex md:w-[6.70rem] overflow-hidden bg-white items-center justify-center zero-box'>
    ...
    <div className='zero-box absolute left-0.5 pointer-events-none h-full'></div>
    {selectedBets[0] && (
...
interface ResultsProps {
  coverage: number
  getNumberColor: (v: number) => string
  outcome: RouletteDisplayResult | null
  ...
  <div className='flex justify-between rounded-xl bg-panel mb-2 h-16 md:h-24 items-center md:mb-4'>
        ...
        {outcome && (
          <motion.div
            ...
            className={`md:text-lg font-medium text-center md:mb-4 p-3 rounded-lg ${
              outcome.kind === 'win' ? 'text-avocado-light' : 'text-error'
            }`}>
            <span
              className={cn('font-bold', {
                'text-mossad': outcome.message.includes('BLACK')
              })}>
              {outcome.message}
            </span>
    ...
    <motion.div
      className={`md:size-16 aspect-square size-10 font-abril rounded-full flex items-center justify-center text-xl md:text-3xl font-bold text-white ${
        selectedNumber !== null ? getNumberColor(selectedNumber) : 'bg-panel-dark'
      }`}
      animate={{
      ...
      <div>
        {totalBet > 0 && (
          <div className='w-16 md:w-32 text-right md:mb-4'>
            ...
            <div className='tracking-tight md:text-sm text-xs opacity-60'>Coverage</div>
          </div>
        )}
      </div>
      <div>
        {totalBet > 0 && (
          <div className='w-16 md:w-32 text-right md:mb-4'>
            ...
            <div className='tracking-tight md:text-sm text-xs opacity-60'>Total Bet</div>
          </div>
        )}
      </div>
    </div>
