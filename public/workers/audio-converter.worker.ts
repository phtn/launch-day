...
if (typeof self !== 'undefined' && 'location' in self && self.location) {
  try {
    const origin = (self as unknown as {location: {origin: string}}).location
      .origin
...
if (!lamejsLoaded) {
  throw new Error(
    `Failed to load lamejs from any of these paths: ${pathsToTry.join(', ')}`,
  )
}
...
declare const lamejs: {
  Mp3Encoder: new (
    ...
    // encodeBuffer can take 1 argument (mono) or 2 arguments (stereo: left, right)
...
interface AudioConversionRequest {
  samples: number[][] | Float32Array[] // Can be arrays or Float32Arrays
  sampleRate: number
  bitrate: number
  duration?: number
}
...
const messageHandler = async (event: MessageEvent<AudioConversionRequest>) => {
  try {
      ...
      if (!Array.isArray(samples)) {
        throw new Error(
          `Invalid samples data: samples is not an array (type: ${typeof samples})`,
        )
      }
      ...
      if (!samples[0]) {
        throw new Error(
          'Invalid samples data: first channel (samples[0]) is undefined or null',
        )
      }
      ...
      const float32Samples = samples.map((channel: number[] | Float32Array) =>
      ...
      let paddedLeft: Int16Array | null = null
      ...
      for (let i = 0; i < leftChannel.length; i += sampleBlockSize) {
        ...
        if (
          chunkIndex % PROGRESS_UPDATE_INTERVAL === 0 ||
          chunkIndex === totalChunks
        ) {
          const progress = Math.floor(
            ((leftChannel.length - remaining) / leftChannel.length) * 100,
          )
        ...
        let leftToEncode: Int16Array
        let rightToEncode: Int16Array | undefined
        ...
        try {
          ...
          console.error(
            `Error encoding chunk ${chunkIndex} at index ${i}:`,
            encodeError,
          )
          throw encodeError
        }
